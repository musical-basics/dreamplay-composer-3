...
export default function AdminDashboard() {
    ...
    const hasAsset = (value?: string | null) => !!value && value.trim().length > 0

    const getAssetPillClass = (isPresent: boolean) =>
        isPresent
                ...
                <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <Link href="/studio/exports">
                            <Button variant="outline" className="bg-zinc-900 text-white border-zinc-700 hover:bg-zinc-800 font-outfit">
                                Exports
                            </Button>
                        </Link>
                        <Link href="/guide">
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px] uppercase tracking-wide">
                                        <span className={`inline-flex items-center rounded-md border px-1.5 py-0.5 ${getAssetPillClass(hasAsset(config.xml_url))}`}>
                                            MusicXML
                                        </span>
                                        <span className={`inline-flex items-center rounded-md border px-1.5 py-0.5 ${getAssetPillClass(hasAsset(config.midi_url))}`}>
                                            MIDI
                                        </span>
                                        <span className={`inline-flex items-center rounded-md border px-1.5 py-0.5 ${getAssetPillClass(hasAsset(config.audio_url))}`}>
                                            WAV
                                        </span>
                                    </div>
                                </div>
