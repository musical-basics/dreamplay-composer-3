...
# Upload Bug: .mxl Files Silently Broken by Extension Rename

**Date:** 2026-04-06

## Symptom
Users uploading `.mxl` files (ZIP-compressed MusicXML, generated by MuseScore, Sibelius, Finale, etc.) saw `Error loading score: OpenSheetMusicDisplay: Invalid MXL file` on their viewer page. `.xml` and `.musicxml` uploads worked fine. The failure appeared in both the viewer and admin view.

## Root Cause
Three compounding issues in the upload pipeline:

1. **Extension stripped on upload:** `api/config-upload/route.ts` hardcoded `fileKey = 'score.xml'` for ALL XML-type uploads regardless of original extension. An `.mxl` file (binary ZIP) was stored in R2 as `score.xml`.

2. **Proxy used content-type to detect MXL:** `api/xml/route.ts` checked if the R2 `Content-Type` response header included `application/vnd.recordare.musicxml` to decide whether to serve binary vs text. If R2 stored the file with that MIME type but the URL ended in `.xml`, the proxy would attempt to serve it as binary — confusing OSMD. Conversely, if the file was stored as `application/octet-stream`, the proxy served ZIP bytes as text — also confusing OSMD.

3. **Browser `file.type` unreliable for XML:** For `.xml` and `.musicxml` files, different browsers and OS versions report different `file.type` values (empty string, `text/xml`, `application/xml`, `application/vnd.recordare.musicxml`). This caused inconsistent content-types to be stored in R2.

## Failed Fix (Attempt 1)
Normalized `file.type` client-side to `application/xml` before uploading, hoping to make R2 content-type consistent.
- **Why it failed:** The fundamental issue was still the extension strip — `.mxl` files were ZIP binary stored as `score.xml`. The XML proxy was still routing them incorrectly.

## Final Solution

### Fix 1 — `api/config-upload/route.ts`: Preserve `.mxl` extension
```typescript
const ext = (originalFileName.split('.').pop() || 'bin').toLowerCase()
fileKey = ext === 'mxl' ? 'score.mxl' : 'score.xml'
// Content-type for MXL forced to the correct MIME type
ContentType: ext === 'mxl' ? 'application/vnd.recordare.musicxml' : contentType
```
`.mxl` files now stored as `score.mxl` in R2 — OSMD's URL-based detection works correctly.

### Fix 2 — `api/xml/route.ts`: MXL only by URL extension, not content-type
```typescript
// Before (broken):
const isMxl = url.endsWith('.mxl') || contentType.includes('application/vnd.recordare.musicxml')
// After (fixed):
const isMxl = urlLower.endsWith('.mxl')
```
Prevents `.xml` files with MusicXML content-type from being misrouted through the binary MXL path.

### Fix 3 — `api/config-upload/route.ts`: Preserve original file for debugging
Each XML upload now generates two presigned URLs: one for the canonical `score.[xml|mxl]` and one for `score_original.[ext]`. The client uploads to both in parallel. Admin can inspect `score_original.*` in Cloudflare R2 to see exactly what the user submitted.

### Fix 4 — Client upload: normalize content-type for XML
```typescript
const getContentType = (f: File, type: 'audio' | 'xml' | 'midi'): string => {
    if (type === 'xml') return 'application/xml'   // never trust file.type for XML
...
## Why It Worked
The root issue was an architectural assumption that all XML-family score files are interchangeable text. MXL is a ZIP format and must be flagged as such end-to-end (storage key, content-type, proxy routing). Once the extension is preserved in R2, OSMD's own URL-based detection handles decompression correctly via `useOSMD`'s Blob-based loading path.

## Files Changed
- `app/api/config-upload/route.ts`
- `app/api/xml/route.ts`
- `app/studio/edit/[id]/page.tsx`
- `app/studio2/edit/[id]/page.tsx`

## Impact on Existing Data
Users who uploaded `.mxl` files before this fix have broken `score.xml` entries in R2 (ZIP binary stored as XML). They must **re-upload** their `.mxl` file to get it stored correctly as `score.mxl`. There is no automated migration path since the original file extension is not stored in the database.
