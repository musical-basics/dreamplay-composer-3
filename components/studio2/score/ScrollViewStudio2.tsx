...
const ScrollViewComponent: React.FC<ScrollViewProps> = ({
    ...
    const calculateNoteMap = useCallback(() => {
        ...
        // Build sets of measure numbers that start or end a repeated section.
        // Try Sheet.Repetitions first (OSMD ≥ 0.9); fall back to per-measure
        // RepetitionInstruction inspection if unavailable.
        ...
        try {
            ...
            for (const rep of repetitions) {
                // RepetitionInstructionEnum: StartRepeat=0 (varies by version) — log to diagnose
                ...
                // Type values: we check both numeric and string forms
        ...
        } catch { /* ignore — we'll try the per-measure fallback */ }

        // Per-measure fallback: inspect SourceMeasure repeat instructions
    ...
    const handleScoreClick = useCallback((event: React.MouseEvent) => {
        ...
        if (clickedMeasureIndex !== -1) {
            ...
            // Combine measure anchor (beat 1) + beat anchors, find closest to click X position
            const allForMeasure: { beat: number; time: number }[] = []
            const measureAnchor = anchors.find((a) => a.measure === measureNumber)
            if (measureAnchor && isValidTimePoint(measureNumber, measureAnchor.time)) {
                allForMeasure.push({ beat: 1, time: measureAnchor.time })
            }
            beatAnchors
                .filter((b) => b.measure === measureNumber && isValidTimePoint(b.measure, b.time))
                .forEach((b) => allForMeasure.push({ beat: b.beat, time: b.time }))
            allForMeasure.sort((a, b) => a.beat - b.beat)

            if (allForMeasure.length > 0) {
                // Use beat X positions to find the closest anchor to the click
